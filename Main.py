...
def normal(): # Don't Remove Code !!!!
        ...
        for i in range(int(jumlah)):
            ...
            autoketik(f"{W}[{G}✓{W}] Berhasil Mengirim Spam")
...
spanduk def():
  ...
  \033[1;0m┌────────────────────────────┼────────────── ─ Dengan ─ ur matikanansansansansansansans posasanarmaskanirim dengan q

  {W}│ Pencipta{R}:{W}AmmarBN Versi {R}:{G} 3.0{W}

  {W}| Tambahan{R}:{W}ILHAM Ganz | {W} |

  {W}│ Github{R}:{G}github.com/IlhAm {W}│ {W}Ip Anda {R}:{Y} {ip} {W}│

  {W}│ Youtube{R}:{W}ILHAM {biru}Ganz {W}│ Perfect Brutal {biru}Alat SMS {W}│

  └─ans───── Chasan 4 Choll 4 Choll 4X 4 Chocanding "" "" 4irim 4irim 4 Chollans 4 4 4 4 Chsans 4 Chsans 4 Chs Chsans 4 4 4 4 Chsans 4 Chsans 4ootoot Miss─ Setolanding Manarm """)
...
def banner2():
    ...
    cetak (f"""{putih}
           ...
           {putih}│ 4{R}.{W}Keluar dari Alat {B} null {W}│

           {putih}└─────────────────────────────┴──────────┘""")

def start(): # Def Untuk Mulai Alat

    spanduk()

    spanduk2()

    pil=input(f" Nomor Masukan : ")
    ...
    jika pil == "3":
        ...
        os.system("xdg-buka https://youtube.com/channel/UCBNU0pIvB_I8ncI92dFYqEw")

        print (f"{Y}${W} Terima Kasih Telah Menyampaikan Eror,Creator Akan Segera Segera")

        tanya()

    jika pil == "4":

        print (f"{W}[{R}!{W}] Keluar Dengan Program")

        sistem.keluar()

start() # Memulai Alat

# Hak Cipta 08/07/22kami
